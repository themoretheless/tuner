...
<template>
  <div class="w-full max-w-[420px]">
    ...
    <svg v-else-if="mode === 'needle'" viewBox="0 0 100 44" class="h-20 w-full" preserveAspectRatio="none">
      <path d="M12 36 Q50 4 88 36" fill="none" stroke="var(--border)" stroke-width="5" stroke-linecap="round" />
      <path d="M43 10 Q50 6 57 10" fill="none" stroke="var(--accent)" stroke-width="3" stroke-linecap="round" opacity="0.55" />
      <line x1="50" y1="36" x2="50" y2="8" stroke="var(--warning)" stroke-width="2.2" stroke-linecap="round" :transform="`rotate(${needleAngle(cents)} 50 36)`" />
      <circle cx="50" cy="36" r="3.6" fill="var(--text-dim)" />
      <text x="12" y="42" fill="var(--text-dim)" font-size="5">-50</text>
      <text x="47" y="42" fill="var(--text-dim)" font-size="5">0</text>
      <text x="84" y="42" fill="var(--text-dim)" font-size="5">+50</text>
    </svg>
    ...
    <svg v-else viewBox="0 0 100 24" class="h-10 w-full rounded-lg border border-slate-800 bg-[#0f1319]" preserveAspectRatio="none">
      <defs>
        <pattern id="strobe-stripes" patternUnits="userSpaceOnUse" width="8" height="24" :patternTransform="`translate(${strobeShift(cents)} 0)`">
          <rect x="0" y="0" width="3" height="24" fill="var(--accent)" :opacity="isInTune ? 0.75 : 0.35" />
        </pattern>
      </defs>
