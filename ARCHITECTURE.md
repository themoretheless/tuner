...
- In-app pipeline diagnostics now cover bounded frame evidence, freeze/replay, A/B and observed latency. A licensed 19-WAV corpus, blocking temporal quality gate, sample-indexed Rust replay and exact shared browser PCM/WAV+JSON v2 capture now exist; cross-backend replay comparison, mic self-test, typed errors, benchmarks, noise/SNR matrices, soak tests and release hardening remain incomplete.
...
The current list contains 300 open/partial `R#` findings and a 94-item closure registry: 103/77 in the original range and 197/17 in the independent post-refactor pass. The Top 500 remains a full idea/risk register; done rows retain dated `[DONE]` markers for traceability.
...
flowchart LR
  ...
  GeneratedMath --> Core
  GeneratedMath --> Tuning["Tuning model + commands + detection"]
...
- `PipelineEvidencePanel`, `PipelineTelemetryPanel`, `PipelineDecisionTimeline`, `PipelineSpectralPanel`, `PipelineFrameInspector` and `PipelineExperimentPanel` are presentation-only modules.
...
1. **Accuracy work starts with evidence.** `pitch-core::quality` owns pure temporal failure metrics; the licensed 19-WAV corpus has verified provenance/checksums, blocking per-scenario thresholds and a CI JSON artifact. Rust replay envelopes and browser PCM/WAV+JSON v2 captures are sample-indexed; automated cross-backend comparison remains the parity gap. Top-K candidates and phase-aware policy remain core contracts, not Vue experiments.
...
We role-played 10 different experts against the pre-refactor design and asked how they would build it from scratch. Their critique text below is the design input, not a claim that every sentence is still true; the response table records the current result.
...
### Critic Response Status

| Critic | 2026-07-11 response | Remaining challenge |
| --- | --- | --- |
| Systems | Session, focused controllers, injected settings/output and segregated feature ports now have owners | Propagate typed lifecycle failures across adapters |
...
| QA | Synthetic, lifecycle, profile, controller, architecture, core, real-WAV and E2E tests shipped | Cross-backend replay, SNR, failure and soak suites |
...
audio-input/
  cpal callback -> bounded recycled chunks -> worker frame callback
...
- Shared musical/DSP truth lives in Rust or generated data. The TS detector is an explicit degraded runtime fallback with cents/confidence/smoothing conformance gates, not a second target architecture.
...
**Status 2026-07-12: structural split and full-frame WASM processor done.** `dsp/{detector,yin,mpm,power}`, `spectrum`, `wasm/{frame,processor}`, `signal`, `smoother`, `frames`, `resolution`, `domain` and `engine` have focused owners; `PitchDetector`, `FrameResolver`, `EngineConfig` and browser `TunerProcessor` exist. Remaining: inject the detector into `TunerEngine` and remove owned spectrum cloning.
...
**Status 2026-07-11: native path done, shell parity partial.** egui consumes pitch-core frames through shared audio input; main/audio/state/visualization are separate; repaint/history are frame-gated and FFT follows visibility. Remaining: explicit native session trait, typed errors and removal of static WASM globals.
...
- Settings persistence watches deeply again, so an in-place mutation of a structured setting (pipeline config, custom tunings, offsets, history) cannot silently stop being saved.
- Display commands validate instead of coercing: an unrecognized theme/layout/display value is ignored rather than switching the user to the default.
- Timed tones always get a release ramp, so a metronome/reference tone with no explicit release no longer ends in a click.
- TS `detectorAgreement` mirrors the Rust arms exactly: two detectors where one carries an unusable frequency count as no evidence, not as a single-source prior.
- Dead code removed: the unreachable `exportCustomTunings`, the `sourceDetectionFrame` alias, and the fictional `unknown | null` detected-note type.

Checked and deliberately not changed: a stored `lastTuningId` that no longer resolves is already normalized at the storage boundary (`normalizePersistedSettings`), so no preference-preserving branch is warranted; `hasDetection`, `cents` and `isInTune` all derive from the same presentation frame, and `hasPriorityTransition` replaces the frame on acquisition/loss, so no boundary flicker exists.
...
3. Close session lifecycle status visibility and native stop-error propagation gaps (`R182`, `R183`, `R193`).
4. Add typed diagnostics/errors and device-loss recovery evidence.
...
81. [DONE 2026-07-11] Complete DPR handling consistently across Waveform, Spectrum, Spectrogram and CentsHistory.
82. [DONE 2026-07-11] Remove all direct analyser.getByte* calls from components; compute or receive data from useVizData.
...
93. [DONE 2026-07-11] Implement Service Worker + cache-first for offline PWA.
...
99. [DONE 2026-07-11] Add vitest fake for useTuner that feeds scripted frames.
100. [DONE 2026-07-11] Lazy-initialize expensive canvases only after first listen or explicit enable.

### egui / Native Rust UI Specific (101-120)
101. [DONE 2026-07-11] Refactor egui/src/main.rs App::update (god method) into smaller methods or separate widgets.
102. [DONE 2026-07-11] Extract painters: WaveformPainter, SpectrumPainter, SpectrogramPainter that take only data slices.
...
112. [DONE 2026-07-11] Fix hardcoded 44100 assumption in any harmonic overlay code.
...
123. [DONE 2026-07-11] Device change handling: restart stream cleanly without full app reload.
124. [DONE 2026-07-11] Expose sample rate actually negotiated; warn if far from 44100/48000.
...
141. [DONE 2026-07-11] Consistent DPR scaling + backing store size logic in every canvas component (finish branch work).
142. [DONE 2026-07-11] Make all canvases respect CSS container queries or ResizeObserver only.
...
154. [DONE 2026-07-11] Dark / light theme toggle with proper canvas palette switch.
155. [DONE 2026-07-11] Animation: only animate needle when actively changing; respect prefers-reduced-motion.
...
162. [DONE 2026-07-11] Capo mode: offset all targets by N frets with visual indicator.
...
167. [DONE 2026-07-11] Built-in chromatic mode (ignore selected tuning, just show nearest note).
168. Support 7-string and 8-string guitar presets + baritone tunings.
169. [DONE 2026-07-11] Bass guitar 4/5/6 string presets with lower freq gate.
170. [DONE 2026-07-11] Ukulele, mandolin, banjo quick presets (keep guitar primary).
171. "Tune to recording": drag short clip or use mic to analyze a reference recording's tuning.
172. [DONE 2026-07-11] Export/Import tuning presets as JSON.
...
177. [DONE 2026-07-11] Show open-string target frequency + detected live side-by-side.
178. [DONE 2026-07-11] Add "just intonation" vs ET deviation display toggle for interested players.
...
186. [DONE 2026-07-11] Colorblind-safe palettes (deuteranopia etc.) selectable in settings.
...
| --- | --- | --- | --- |
| Single Responsibility | One module owns one workflow | Application/tuning/settings/output owners are split; lifecycle error ownership is still fragmented | Introduce typed stop/start results shared by adapters and session |
...
| Dependency Inversion | UI depends on capabilities, not APIs | Settings storage, fullscreen and output audio are injected; native stop results still disappear inside an adapter | Return typed lifecycle results and surface them through session commands |
...
| Done | Practice workflow | `domain/practice.ts`, `domain/metronome.ts`, `useEarTraining.ts`, `adapters/vue/controllers/practiceController.ts` | Playback stays behind `AudioOutputPort`; keep scheduler independent of Vue |
...
| Done | Presentation slices | `adapters/vue/capabilities/*`, `adapters/vue/ports/*`, `app/featurePorts.ts`, `features/*` | Keep views from importing broad composables; boundary test enforces this |
| Done | egui decomposition | `app/audio/state/visualization.rs` | Add painter smoke tests, not another structural split |

### Refactor Order By Bite Size

1. `[done]` Serialize `start/stop/restart/fail` with explicit session status.
2. `[done]` Add one TS `AudioInputPort` for web/native/synthetic/file adapters with contract and session tests.
...
5. `[done]` Extract practice pure logic and command ownership into its application controller.
6. `[done]` Add per-feature ports, custom-library controller and separate tuning model/commands/detection owners.
7. `[done]` Split Rust pitch range, detector, smoother, signal and spectrum modules.
8. `[done]` Add `PitchDetector` in Rust.
9. `[done]` Move spectrum analyzer out of engine and gate it by demand.
10. `[done]` Introduce strict versioned `UserProfileV1`.
11. `[done]` Generate web/Rust tuning data from one workspace registry.
12. `[done]` Split `App.vue` into Tuner, Library, Practice and Analysis feature shells.
13. `[done]` Make settings factory-owned/injectable and reduce `useTuner` to a compatibility facade over the tested application composition root.
14. `[done]` Inject one web output-audio port; split metronome math, sample-clock scheduling, playback adapter and Vue state.
15. `[done]` Isolate Vue under `adapters/vue`, replace stateful tuning computed with an explicit domain machine, and remove the concrete application service bag.

### Finer-Grained Piece Map (36 pieces, for R181+)

The 10-slice table above is the architectural target; the table below is the **navigational unit** — small enough that reading one piece's target files plus its findings in [recommendation.md](recommendation.md) (section "Post-Refactor Findings (R181-R394, by SOLID/DRY Piece)") is a single sitting. It reflects the current post-refactor file layout exactly (e.g. `pitch-core/src/dsp/` is now four files, egui is now four files, web gained `ports/`, `session/`, `settings/`, `domain/`, `features/`). Each piece maps onto one or more of the 10 slices above.

**Web State & Session (Vue composables + new ports/session/settings/domain layers)**

- `Q1` **application composition** — web/src/adapters/vue/useTunerApplication.ts and explicit capabilities; web/src/composables/useTuner.ts is a compatibility export
- `Q2` **sessionLifecycle state machine** — web/src/session/sessionLifecycle.ts (2 new findings in recommendation.md, R181+)
- `Q3` **useTunerSession** — web/src/composables/useTunerSession.ts (3 new findings in recommendation.md, R181+)
- `Q4` **audioInput port contract** — web/src/ports/audioInput.ts (2 new findings in recommendation.md, R181+)
- `Q5` **useAudioInput (web adapter)** — web/src/composables/useAudioInput.ts (2 new findings in recommendation.md, R181+)
- `Q6` **native + synthetic audio adapters** — web/src/composables/useNativeAudioInput.ts, useSyntheticAudioInput.ts (4 new findings in recommendation.md, R181+)
- `Q7` **output audio + reference tone** — web/src/ports/audioOutput.ts, web/src/platform/webAudioOutput.ts, web/src/composables/useReferenceTone.ts
- `Q8` **metronome** — web/src/domain/metronome.ts, web/src/application/services/metronomeScheduler.ts, web/src/composables/useMetronome.ts
- `Q9` **practice domain/application** — web/src/domain/practice.ts, web/src/composables/useEarTraining.ts, web/src/adapters/vue/controllers/practiceController.ts
- `Q10` **settings/profile persistence** — web/src/settings/settingsState.ts, web/src/composables/useSettings.ts, web/src/settings/normalizeSettings.ts, web/src/settings/profileCodec.ts
- `Q11` **tuning domain/application** — web/src/domain/tuningTracking.ts, tuningDetectionMachine.ts, web/src/composables/useTuningModel.ts, useTuningDetection.ts, web/src/application/controllers/tuning*Commands.ts
- `Q12` **feature contracts** — web/src/application/ports/value.ts, web/src/adapters/vue/capabilities/*.ts, web/src/adapters/vue/ports/*.ts, web/src/app/featurePorts.ts

**Music / Pitch Domain (TS + Rust pitch-core, now split into dsp/*)**

- `Q13` **music/note domain (TS)** — web/src/utils/notes.ts (5 new findings in recommendation.md, R181+)
- `Q14` **pitch-core::domain** — pitch-core/src/domain.rs
- `Q15` **pitch-core::dsp::yin** — pitch-core/src/dsp/yin.rs (3 new findings in recommendation.md, R181+)
- `Q16` **pitch-core::dsp::mpm** — pitch-core/src/dsp/mpm.rs (2 new findings in recommendation.md, R181+)
- `Q17` **pitch-core::dsp::power** — pitch-core/src/dsp/power.rs
- `Q18` **pitch-core::dsp::detector + PitchDetector trait** — pitch-core/src/dsp/detector.rs, pitch-core/src/dsp/mod.rs (4 new findings in recommendation.md, R181+)
- `Q19` **pitch-core::spectrum** — pitch-core/src/spectrum.rs
- `Q20` **pitch-core::engine/smoother/signal/frames** — pitch-core/src/engine.rs, smoother.rs, signal.rs, frames.rs (7 new findings in recommendation.md, R181+)
- `Q21` **wasm bindings + web pitch worker adapter** — pitch-core/src/wasm.rs, web/src/workers/pitchCoreAdapter.ts, web/src/workers/pitchWorker.ts (6 new findings in recommendation.md, R181+)

**Visualization / Canvas (Web + egui)**

- `Q22` **visualization frame contracts** — web/src/composables/useVisualizationFrames.ts, web/src/utils/canvasPalette.ts (7 new findings in recommendation.md, R181+)
- `Q23` **canvas renderer/DPR** — web/src/composables/useCanvasRenderer.ts, useHiDpiCanvas.ts (6 new findings in recommendation.md, R181+)
...
- `Q30` **test coverage & CI wiring** — web/tests/*, web/e2e/*, pitch-core/tests/pitch_core.rs, .github/workflows/* (25 new findings in recommendation.md, R181+)
- `Q31` **build/PWA/offline pipeline** — web/src/platform/serviceWorker.ts, web/vite.config.ts, web/package.json, web/public/manifest.webmanifest (13 new findings in recommendation.md, R181+)

**Product / UX / Visual Design (now 4-screen feature architecture)**

- `Q32` **design system (tokens/typography/color)** — web/src/style.css, web/src/utils/canvasPalette.ts (12 new findings in recommendation.md, R181+)
- `Q33` **interaction design & feature-view states** — web/src/features/tuner/LiveTunerView.vue, features/practice/PracticeView.vue, features/library/LibraryView.vue, features/analysis/AnalysisView.vue (9 new findings in recommendation.md, R181+)
- `Q34` **information architecture / 4-screen execution** — web/src/App.vue, web/src/app/featurePorts.ts (8 new findings in recommendation.md, R181+)
- `Q35` **accessibility-as-design (cross-cutting)** — web/src/features/*/*.vue, web/src/components/*.vue, web/src/style.css (6 new findings in recommendation.md, R181+)
- `Q36` **content/copy & localization** — web/src/stores/l10n.ts, egui/src/*.rs strings (10 new findings in recommendation.md, R181+)
...
- **Analysis:** opt-in waveform/spectrum/spectrogram, cents history and display/accessibility modes.

Audio/A4 stay near the live task; instrument/temperament/profile settings stay in Library; display settings stay in Analysis. A separate Settings route should be added only when diagnostics/release controls make these surfaces too dense.
...
| M127 | P3 | dx-quality | DSP scope-recorder: dump per-frame internals to a replayable .ndjson trace | [PARTIAL 2026-07-19] Rust sample-indexed JSON and exact shared browser PCM/WAV+JSON v2 exist; automated cross-backend replay comparison remains. |
...
- [recommendation.md](recommendation.md) - current grounded extract: 300 open/partial and 94 closed stable `R#` references.

Key highlights that directly block the target architecture:
- Add automated cross-backend replay over exact PCM envelopes.
- Close lifecycle status/stop-error propagation gaps and add typed diagnostics (`R182`, `R183`, `R193`).
