...
33. **P1: Core tests are improved but still synthetic-heavy.** Vitest covers lifecycle/profile/domain flows; Rust and browser WASM share cents-budget fixtures; Playwright covers synthetic E2 and responsive Library behavior. Real recordings, permission/device-loss flows, noise/SNR sweeps and soak tests remain.
...
Verified closed master items in this pass: **M1, M2, M5, M6, M7, M13, M22, M24, M25, M26, M29, M39, M40, M41, M44, M48, M49, M50, M51, M59, M64, M65, M68, M70, M71, M177**.
...
## Summary
- This file contains 118 current open/partial `R#` findings (`R1`-`R180` range) and a 62-item stable closure registry from the three-iteration refactor.
- A second, independent post-refactor pass added **214 more `R#` findings (`R181`-`R394`)**, organized by 36 finer SOLID/DRY pieces — see "Post-Refactor Findings" above. **332 open items total.**
...
- Highest impact now is: native frame/smoothing semantics, file/WAV input, remaining controller splits, real-audio reliability/performance tests, diagnostics and release hardening — plus, from the new pass, the `sessionLifecycle` status-visibility gap (R182), the asymmetric `AudioInputPort` contract (R187-R188), and the native `stop()` failure being swallowed twice over (R183, R193).
